...
like( $out, qr/result: .*rpmrebuild-.*-1\.noarch\.rpm/, 'filter control' )
  or diag("out=$out\n");

# -f, --filter is an alias of --change-spec-whole
$out =
  `$cmd --filter='sed -e "s/^Release:.*/Release: yourelease/"' rpmrebuild 2>&1`;
like(
	$out,
	qr/result: .*rpmrebuild-.*-yourelease\.noarch\.rpm/,
	'filter long build'
) or diag("out=$out\n");

# -f
$out = `$cmd -f 'sed -e "s/^Release:.*/Release: yourelease/"' rpmrebuild 2>&1`;
like(
	$out,
	qr/result: .*rpmrebuild-.*-yourelease\.noarch\.rpm/,
	'filter short build'

...
# demofiles.plug
# output list of files (by ls)

# 1-2 control without
my $out = `$cmd rpmrebuild 2>&1 `;
like( $out, qr/result:.*rpmrebuild.*.rpm/,
	'plugin demofiles without check build' )
  or diag("out=$out\n");
unlike( $out, qr/demofiles.plug.1rrp/, 'plugin demofiles without check output' )
  or diag("out=$out\n");

# 3-4 with plugin
$out = ` $cmd --include $plug_dir/demofiles.plug rpmrebuild 2>&1 `;
like( $out, qr/result:.*rpmrebuild.*.rpm/, 'plugin demofiles check build' )
  or diag("out=$out\n");
like( $out, qr/demofiles.plug.1rrp/, 'plugin demofiles check output' )
  or diag("out=$out\n");
